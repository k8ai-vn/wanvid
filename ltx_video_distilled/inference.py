...
# Tạo volume cho cache
cache_volume = Volume.from_name(
    "huggingface-cache",
...
def create_ltx_video_pipeline(
    ...
    # Create cache directory for text encoder and tokenizer
    cache_dir = os.path.join(MODEL_DIR, "text_encoder_cache")
    os.makedirs(cache_dir, exist_ok=True)
    
    # Check if models exist in cache
    text_encoder_cache_path = os.path.join(cache_dir, "text_encoder")
    tokenizer_cache_path = os.path.join(cache_dir, "tokenizer")
    
    if os.path.exists(text_encoder_cache_path) and os.path.exists(tokenizer_cache_path):
        print("Loading text encoder and tokenizer from cache...")
        text_encoder = T5EncoderModel.from_pretrained(text_encoder_cache_path)
        tokenizer = T5Tokenizer.from_pretrained(tokenizer_cache_path)
    else:
        print("Downloading text encoder and tokenizer from Hugging Face...")
        ...
        print("Saving text encoder and tokenizer to cache...")
        text_encoder.save_pretrained(text_encoder_cache_path)
